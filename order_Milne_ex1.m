clc;
clear;
fprintf("\n Milne method of step 4 to solve y'=te^(3t)-2y, y(t0)=y0. :\n");
...
w(1)=y0;
for i=4:N(n)
    w(i+1)=w(i-3)+(4*h(n)/3)*(2*f(t(i),w(i))-f(t(i-1),w(i-1))+2*f(t(i-2),w(i-2)));
end
...
title('Order of convergence for Milne-4 method', 11)
 xlabel('h-->')
