...
% Modified Euler method (RK method of order two)
t=t0:h:T;
w=zeros(1,N+1);
w(1)=y0;
for i=1:N
...
% Error computation
 ...
 D=['         t     ' '     Exact(t)  ' '   Modified Euler soln.    ' '    Error.   ' ];
 disp(D);
 ...
 % Plotting Modifed Euler solution
 plot(t, y, 'ko');
 plot(t, w, '--g*');

 title('Modified Euler method (RK of order two)', 11)
 xlabel('t-->')
 ylabel('y(t)-->')
 % xlim([-0.2 1.2])
 % ylim([-.3 4])
legend('Exact Soln.','Data points on exact soln.','Modified Euler Approx. soln.', 'Location', 'northwest')
grid on