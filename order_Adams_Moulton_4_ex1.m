...
for i=5:N(n)
    w(i+1)=(720/(720+251*h(n)*2))*(w(i)+(h(n)/720)*(251*(t(i+1)*exp(3*t(i+1)))+646*f(t(i),w(i))-264*f(t(i-1),w(i-1))+106*f(t(i-2),w(i-2))-19*f(t(i-3),w(i-3))));
end
...
title('Order of convergence for Adams-Moulton-4 method', 11)
 xlabel('h-->')
 ylabel('Global Error(h)-->')
 % xlim([-0.2 1.2])
 % ylim([-.3 4])
legend('Order of convergence line','Reference line of slope 5', 'Location', 'northwest')