clc;
clear;
fprintf("\n Predictor- Corrector method of order 4 to solve y'=te^(3t)-2y, y(t0)=y0. :\n");
...
for i=4:N
    % Predictor Adams-Bashforth algorithm of order 4
    w(i+1)=w(i)+(h/24)*(55*f(t(i),w(i))-59*f(t(i-1),w(i-1))+37*f(t(i-2),w(i-2))-9*f(t(i-3),w(i-3)));
    % Corrector Adams-Moulton algorithm of order 3
...
% Error computation
 ...
 title('Predictor-Corrector method of order four', 11)
 xlabel('t-->')
 ylabel('y(t)-->')
 % xlim([-0.2 1.2])
 % ylim([-.3 4])
legend('Exact Soln.','Data points on exact soln.','PC Approx. soln.', 'Location', 'northwest')
grid on