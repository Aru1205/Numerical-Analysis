clc;
clear;
fprintf("\n Adams-Moulton method of order 4 to solve y'=te^(3t)-2y, y(t0)=y0. :\n");
...
% Adams-Moulton algorithm of order 4
for i=5:N
    w(i+1)=(720/(720+251*h*2))*(w(i)+(h/720)*(251*(t(i+1)*exp(3*t(i+1)))+646*f(t(i),w(i))-264*f(t(i-1),w(i-1))+106*f(t(i-2),w(i-2))-19*f(t(i-3),w(i-3))));
end
...
% Error computation
 ...
 title('Adams-Moulton method of order four', 11)
 xlabel('t-->')
 ylabel('y(t)-->')
 % xlim([-0.2 1.2])
 % ylim([-.3 4])
legend('Exact Soln.','Data points on exact soln.','AM Approx. soln.', 'Location', 'northwest')
grid on